 ...
 *     acquire/try/release.
 *   - **On-disk codec.** {@link normalizeIndex} turns an arbitrary parsed
 *     JSON value into a v3 task list, migrating v1/v2 manifests by stripping
 *     dropped fields and self-healing legacy status rows; the per-row half
 *     ({@link coerceTask}) lives in `task-coerce.ts`.
...
import { coerceTask } from "./task-coerce.ts"
