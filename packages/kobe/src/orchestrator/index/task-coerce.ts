/**
 * Per-row codec for the task manifest: {@link coerceTask} turns one parsed
 * JSON entry into a v3 {@link Task}, tolerating v1/v2 shapes and dropping
 * malformed optional sub-records instead of the whole row. The manifest-level
 * codec (versions, tombstones, disk merge) lives in `store-codec.ts`.
 */
