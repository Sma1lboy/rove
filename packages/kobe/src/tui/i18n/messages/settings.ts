...
export const en = {
  ...
  autoEffort: {
    ...
    classifierTitle: "Auto routing",
    classifierHint:
      "The table above says what each depth RUNS. This routes a task TO one: switch it on and `rove api add --tier auto` reads the task's first message and picks the depth for you. It is off by default, it is never required, and it can never fail a create — no key, no network, a timeout, or an answer it is not confident enough about all leave you on the depth you would have had.",
    classifierDataFlow:
      "! Turning routing on sends the task's first message to a third party that is NOT the engine vendor you picked — TypeSafe System One for `jev`, or whatever address you point `custom` at. It reads the text before your engine does. Nothing leaves this machine while it is off.",
    classifierLabel: "Route with",
    classifierOff: "off",
    classifierCustomLabel: "custom",
    classifierOffHint: "— no routing; nothing is sent anywhere",
    ...
    endpointUnset: "(none — enter one, then set Route with to custom)",
    endpointTitle: "Custom routing endpoint",
    endpointField: "URL",
    endpointInvalidTitle: "Not a URL",
    endpointInvalidBody: "The endpoint has to start with http:// or https://. Leave it empty to clear it.",
    thresholdLabel: "Confidence floor",
    thresholdHint: "— below this, the task is not routed",
    ...
    keyNoVariableBody:
      "A custom routing endpoint is sent no Authorization header until autoEffort.classifierCustomKeyEnv names the variable its key lives in. Set that first — a key stored under a name nothing reads is a key at rest for no purpose.",
    keyWriteFailedTitle: "Key not saved",
    keyWriteFailedBody: "~/.rove/secrets.json could not be written: {reason}",
    keyMissing: "! no key — nothing is routed and tasks keep their usual depth",
...
export const zh: typeof en = {
  ...
  autoEffort: {
    ...
    classifierTitle: "自动路由",
    classifierHint:
      "上面那张表说的是每一档**跑什么**，这里决定一个任务**被路由到哪一档**：打开之后，`rove api add --tier auto` 会读任务的第一句话替你选档。默认关着，从来不是必需的，也永远不会让建 task 失败——没 key、断网、超时、或者它自己没把握，都只是让你停在本来就会用的那一档上。",
    classifierDataFlow:
      "! 打开路由，任务的第一句话会发给一个**不是你选的那个引擎厂商**的第三方——`jev` 是 TypeSafe（api.typesafe.ai），`custom` 是你自己填的地址。它比你的引擎更早读到这段文字。关着的时候什么都不发。",
    classifierLabel: "路由方式",
    classifierOff: "关",
    classifierCustomLabel: "自定义",
    classifierOffHint: "——不路由，也不向任何地方发送",
    classifierJevHint: "——TypeSafe System One，走你自己的 key 计费",
    classifierCustomHint: "——你自己的接口：POST 那段文本，返回一个档位加一个置信度",
    endpointLabel: "接口地址",
    endpointUnset: "（未填——先填一个，再把「路由方式」切到自定义）",
    endpointTitle: "自定义路由接口",
    endpointField: "URL",
    endpointInvalidTitle: "不是一个 URL",
    endpointInvalidBody: "接口地址要以 http:// 或 https:// 开头。留空则清除。",
    thresholdLabel: "置信度下限",
    thresholdHint: "——低于这个值就不做路由",
    ...
    keyNoVariableBody:
      "在 autoEffort.classifierCustomKeyEnv 指定存 key 的变量之前，自定义路由接口不会收到任何 Authorization 头。先设那个——存在一个没人读的名字下面的 key，只是白白落在盘上。",
    keyWriteFailedTitle: "key 没存下",
    keyWriteFailedBody: "写不了 ~/.rove/secrets.json：{reason}",
    keyMissing: "! 没有 key——不会做路由，任务保持原来的档位",
