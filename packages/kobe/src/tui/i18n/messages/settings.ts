...
export const en = {
  ...
  autoEffort: {
    ...
    classifierTitle: "Tier classifier",
    classifierHint:
      "The table above says what each depth RUNS. This says who PICKS one: with a classifier set, `rove api add --tier auto` reads the task's first message and fills the depth for you. It is off by default, it is never required, and it can never fail a create — no key, no network, a timeout, or an answer it is not confident enough about all leave you on the depth you would have had.",
    classifierDataFlow:
      "! Turning this on sends the task's first message to a third party that is NOT the engine vendor you picked — TypeSafe System One for `jev`, or whatever address you point `custom` at. It reads the text before your engine does. Nothing leaves this machine while it is off.",
    ...
    thresholdHint: "— below this, no depth is pre-filled",
    ...
    keyMissing: "! {env} is not set — the classifier stays silent and tasks keep their usual depth",
    keyPresent: "● key read from {env}",
  },
  keybindings: {
    title: "Keybindings",
...
export const zh: typeof en = {
  ...
  autoEffort: {
    ...
    classifierTitle: "档位分类器",
    classifierHint:
      "上面那张表说的是每一档**跑什么**，这里说的是**谁来选**：配了分类器之后，`rove api add --tier auto` 会读任务的第一句话替你填档。默认关着，从来不是必需的，也永远不会让建 task 失败——没 key、断网、超时、或者它自己没把握，都只是让你停在本来就会用的那一档上。",
    classifierDataFlow:
      "! 打开它，任务的第一句话会发给一个**不是你选的那个引擎厂商**的第三方——`jev` 是 TypeSafe（api.typesafe.ai），`custom` 是你自己填的地址。它比你的引擎更早读到这段文字。关着的时候什么都不发。",
    classifierLabel: "分类器",
    classifierOff: "关",
