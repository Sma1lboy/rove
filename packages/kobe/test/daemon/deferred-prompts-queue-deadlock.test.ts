...
describe("deferred-prompts store queue under an in-flight claim", () => {
   ...
   * A discard is a SOFT delete: the record stays until its ordinary TTL so
   * `deferred-release` can still deliver a message dismissed by mistake, so
   * "gone" means `dismissedAt` is set, not absent from the file.
   */
  async function recordsOnDisk(): Promise<{ id: string; dismissedAt?: string }[]> {
    const parsed = JSON.parse(await readFile(path, "utf8")) as {
      records: { id: string; dismissedAt?: string }[]
    }
    return parsed.records
  }

  /** Ids still awaiting delivery — the dismissed ones no longer count. */
  async function liveIdsOnDisk(): Promise<string[]> {
    return (await recordsOnDisk()).filter((r) => r.dismissedAt === undefined).map((r) => r.id)
  }
  ...
  it("keeps serving the store while a discard waits on a live claim", async () => {
    ...
    expect(await liveIdsOnDisk()).toEqual([])
    // Dismissed, not erased — the text survives for a mistaken dismiss.
    expect((await recordsOnDisk()).map((r) => r.id)).toEqual([held.id])
  })
  ...
  it("never drops a claimed record out from under its claim", async () => {
    ...
    expect(await liveIdsOnDisk()).toEqual([claimedRecord.id, sibling.id])
  })
  ...
  it("drops exactly the record it names when no claim is in flight", async () => {
    ...
    expect(await liveIdsOnDisk()).toEqual([keep.id])
    // Dismissing twice is idempotent: it returns the record it already marked.
    const again = await store.discard(target.id, "Inbox item dismissed")
    expect(again?.id).toBe(target.id)
    expect(again?.dismissedAt).toBe((await recordsOnDisk()).find((r) => r.id === target.id)?.dismissedAt)
  })
})
