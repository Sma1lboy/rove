...
describe("nextField / firstFieldFor (per-tab field cycling)", () => {
  it("cycles the existing tab: tabs → engine → repo → baseRef → confirm → tabs", () => {
    ...
    // follows the rows top to bottom: model sits above effort (owner
    // 2026-09-18), so tabbing off the engine reaches the model first.
    expect(nextField("engine", "existing", { effortVisible: true, modelVisible: true })).toBe("model")
    expect(nextField("engine", "existing", { effortVisible: true })).toBe("effort")
    expect(nextField("model", "existing", { effortVisible: true })).toBe("effort")
    expect(nextField("model", "existing")).toBe("repo")
