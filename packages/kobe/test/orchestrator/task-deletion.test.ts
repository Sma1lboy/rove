...
describe("session teardown before worktree removal", { timeout: DURABLE_WRITE_TIMEOUT_MS }, () => {
  it("ends the session and waits for it BEFORE git removes the directory", async () => {
    // The order is the fix: the session used to be killed by the client after
    // the delete RPC returned, racing this runner — and on Windows a process
    // whose cwd is the worktree makes the directory undeletable, so the race's
    // loser was always the removal.
    const order: string[] = []
    ...
    orch = new Orchestrator({
      ...
      worktrees: {
        ...worktrees,
        remove: vi.fn(async () => {
          order.push(`remove:sessionEnded=${sessionEnded}`)
        }),
      } as unknown as GitWorktreeManager,
      tearDownSession: async (id) => {
        order.push(`teardown:${String(id)}`)
        // A kill that is still in flight when remove() runs is the bug.
        await new Promise((resolve) => setTimeout(resolve, 20))
        sessionEnded = true
      },
    })
    const task = await makeTask("/wt/ordered")

    await orch.prepareTaskDeletion(task.id)
    await orch.beginTaskDeletion(task.id)
    await orch.finishTaskDeletion(task.id)

    expect(order).toEqual([`teardown:${task.id}`, "remove:sessionEnded=true"])
    expect(orch.getTask(task.id)).toBeUndefined()
  })

  it("a session host that cannot be asked never blocks the removal the user confirmed", async () => {
    orch.dispose()
    orch = new Orchestrator({
      store,
      worktrees: worktrees as unknown as GitWorktreeManager,
      tearDownSession: async () => {
        throw new Error("pty host unreachable")
      },
    })
    const task = await makeTask("/wt/host-down")

    await orch.prepareTaskDeletion(task.id)
    await orch.beginTaskDeletion(task.id)
    await orch.finishTaskDeletion(task.id)

    expect(worktrees.remove).toHaveBeenCalledWith("/wt/host-down", expect.anything())
    expect(orch.getTask(task.id)).toBeUndefined()
  })

  it("a dir task's deletion asks no session host — nothing on disk is about to go", async () => {
    const torn: string[] = []
    orch.dispose()
    orch = new Orchestrator({
      store,
