 ...
 *
 * No network: `fetch` is injected. No ambient environment either — `env` AND
 * `readSecret` are passed in every case, so neither a real
 * `TYPESAFE_API_KEY` exported in the shell nor one stored in the operator's
...
describe("classifyTier — every failure is a decline", () => {
  ...
  it("sends nothing to a refused endpoint, and says why", async () => {
    ...
    const out = await classifyTier("x", config({ mode: { kind: "refused", why: "use https://" } }), {
      fetch: fn,
      env,
      readSecret: secrets(),
    })
  ...
  it("uses a key from the secrets file when the environment has none", async () => {
    // The stored key's whole reason for existing: a long-lived TUI cannot be
    // handed an env var after it started, so `~/.rove/secrets.json` is the
    // only way that process ever gets one.
    const { fn, calls } = stubFetch(choiceAnswer())
    const out = await classifyTier("x", config(), { fetch: fn, env: {}, readSecret: secrets("apikey_stored") })
    expect(out).toMatchObject({ kind: "picked" })
    expect(headersOf(calls[0]).authorization).toBe("Bearer apikey_stored")
  })
