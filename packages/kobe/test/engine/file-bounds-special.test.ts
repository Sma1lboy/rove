...
it.skipIf(process.platform === "win32")("rejects FIFOs without blocking async or synchronous readers", async () => {
  ...
  const detectorPath = path.resolve("src/engine/turn-detector.ts")
  const script = `import {readTextFileBounded, readTextFileSyncBounded, readFirstLineBounded} from ${JSON.stringify(modulePath)};
    import {CodexTurnDetector, ClaudeTurnDetector} from ${JSON.stringify(detectorPath)};
    const path = ${JSON.stringify(file)};
    console.log(JSON.stringify([await readTextFileBounded(path), readTextFileSyncBounded(path), await readFirstLineBounded(path), await new CodexTurnDetector().latestActivityInFile(path), await new ClaudeTurnDetector().latestActivityInFile(path)]));`
  const child = spawnSync("bun", ["-e", script], { timeout: 3000, encoding: "utf8" })
  expect(child.error).toBeUndefined()
  expect(child.status).toBe(0)
  expect(JSON.parse(child.stdout)).toEqual(["", null, "", null, null])
})
