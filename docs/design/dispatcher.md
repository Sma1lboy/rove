...
| `session.deliver` RPC | `kobe-daemon/src/daemon/handlers-ui.ts` | Delivers. Pastes into the task's live hosted engine session through the same adapter `send` uses (never spawns, respects the delivery guard) and reports `delivered`. Only a `no-session` miss falls through to the channel below — publishing after a successful paste would make a listening browser paste it twice. |
