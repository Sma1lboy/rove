...
> **这一页原本是 [PR #1074](https://github.com/Sma1lboy/rove/pull/1074) 的交付物**，
> 随「auto effort 改名 auto-routing」一起搬进本仓库，因为 #1074 不合入而它的量化结论
> 需要有个正本。改动只有三类：`auto effort` → `auto-routing` 的改名、指向仓库内
> [`auto-routing/`](./auto-routing/) 的链接，以及下面这段现状说明。结论和数字一个字没动。
>
> **判据现在在仓库里，而且是代码的来源。** 当时这一页说判据留在一个私有仓库、
> Rove 只引用它；实现落地时改成了：判据快照进 [`auto-routing/`](./auto-routing/)，
> `scripts/build-tier-rubric.ts` 从那三个文件生成
> `src/engine/tier-rubric.generated.ts`，产品发出去的就是它。
...
分类目标沿用已有的那条规范
（[`auto-routing/rubric-annotator-v5.md`](./auto-routing/rubric-annotator-v5.md)；
写这一页时它还只在一个私有仓库里，现在仓库里这份就是正本）：**用户的指令里，有多少流程需要模型自己推理出来。**
...
```mermaid
flowchart LR
  P["用户写的第一句话"] --> C{"autoRouting.classifier"}
  C -->|off，默认| M["用户自己选档"]
...
**接入口，不自带模型。** 先做 `autoRouting.classifier = off | jev | <url>`，
