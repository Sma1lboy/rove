...
> **同样来自 [PR #1074](https://github.com/Sma1lboy/rove/pull/1074)**，随改名搬进来。
> 下文「快照」现在是本仓库的正本：[`auto-routing/`](./auto-routing/) 里的
> `rubric-annotator-v5.md` / `jev-shots.json` / `jev-head.py` 就是
> `src/engine/tier-rubric.generated.ts` 的来源，改前者、跑 `bun run build:rubric`、
> 提交生成结果，是改判据的唯一路径。仍然没有 remote 的是**标注集**
> （`data/golden/**`，144 条手工标注），见 issue #25。

配套 [`auto-routing-classifier.md`](./auto-routing-classifier.md)（那份定的是**要不要做**，
这份写的是**怎么做**）。目标是让接手的人不用重新趟一遍。

代码和判据在 [`auto-routing/`](./auto-routing/) —— 现在改这里就是改产品行为（经生成器）。
...
4. 已知 rubric 自身有一处打架：`local-0073`（给了 SKILL.md 链接 + "用这个 skill"）
   ...
   73.0% 里有一部分不是模型的错。
5. 产品侧的决定见 [`auto-routing-classifier.md`](./auto-routing-classifier.md)：
   建议做成可插拔的 `autoRouting.classifier = off | jev | <url>`，默认关，
