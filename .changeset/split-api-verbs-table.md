...
Every verb, flag, and handler is unchanged. One user-visible difference: the order verbs are LISTED in by `rove api schema` and `--help` now follows the documented domain grouping (discovery → reads → create → drive → feedback → issues/routines/work-items → edit → lifecycle → worktree), so a few verbs moved position in that listing. Lookup is by name and is unaffected.
