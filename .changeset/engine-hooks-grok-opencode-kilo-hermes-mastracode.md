...
Separately: a hook whose engine writes its payload and then waits for the hook to exit — without closing the pipe — no longer loses that payload. The read used to end only at EOF, so it timed out and discarded bytes that had been sitting in the buffer the whole time; the hook still fired and still exited 0, just carrying no session id and no cwd. It now stops as soon as what it has read is a complete document, which costs an engine that does close the pipe nothing.
