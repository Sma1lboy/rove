...
- `model`: a faster tier than the session's (e.g. `sonnet`) — explorers locate and trace; the explainer does the reasoning
- Read-only: say in the prompt that it must not edit files (the Agent tool has no read-only flag)
...
- `subagent_type`: `general-purpose`
- `model`: omit — it inherits the session's model
...
After the explanation is complete, spawn one architectural critic per `model` value the Agent tool accepts, all in a single message — different models, different blind spots.

For each critic:
- `subagent_type`: `general-purpose`
- `model`: a different one per critic
- Read-only: say so in the prompt
