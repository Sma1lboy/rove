...
3. Pick the runners. Spawn one per design direction, varying `model` across them where the Agent tool allows it. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
...
After all Phase B candidates complete, pick a cross-judge. Prefer a different model from the one that generated the candidates, so the judge does not share their blind spots. Spawn one judge subagent on that model and tell it in its prompt not to edit files. It sees the rubric and the candidates by path label, scores each criterion, and recommends a base with rationale. It runs in parallel with the parent's reading in Phase D, not with the candidates themselves. Spawning while candidates are still writing means the judge sees partial or empty outputs and reports them as dropouts.
