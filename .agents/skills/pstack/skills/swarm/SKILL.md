...
4. Workers inherit the session model by default. Pass `model` on the Agent call only when a cheaper tier clearly fits mechanical work, or when the point is a model race, in which case name each arm's model up front.
