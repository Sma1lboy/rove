...
At the end of the run, before handing back, check the log told the truth. Read this run's own transcript if the harness exposes one. Never glob across other sessions' transcript directories; that reads unrelated private chats. Walk the log against what actually happened:
